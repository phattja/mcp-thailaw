...
import { adToBe, beToAd } from "./calendar.js";
...
function hitToDekaResult(hit: QdrantHit): DekaQdrantResult {
  ...
  return {
    ...
    year: year === undefined ? undefined : adToBe(year),
...
export async function performDekaCollectionSearch(
  ...
  const yearBe = typeof yearRaw === "number"
    ? yearRaw
    : (typeof yearRaw === "string" && /^\d+$/.test(yearRaw.trim()) ? Number(yearRaw.trim()) : undefined);
  const year = yearBe === undefined ? undefined : beToAd(yearBe);
  const cacheArgs = {
    query,
    top_k: topK,
    score_threshold: scoreThreshold,
