...
Common settings:
- \`--qdrant-url\` / \`QDRANT_URL\`: Qdrant base URL (default http://qdrant:6333)
...
- \`--embedding-url\` / \`EMBEDDING_URL\`: llama.cpp embeddings (default http://ai-tool:3003)
- \`--embedding-model\` / \`EMBEDDING_MODEL\`: Embedding model name (default bge-m3-multi)
- \`--colbert-url\` / \`COLBERT_URL\`: ColBERT token vectors (default http://ai-tool:3003)
- \`--vector-mode\` / \`THAILAW_VECTOR_MODE\`: \`colbert\` (default) or \`dense\`
- \`--embedding-api-key\` / \`EMBEDDING_API_KEY\`: Optional bearer token
- \`--rerank-url\` / \`RERANK_URL\`: llama.cpp rerank (default http://ai-tool:3003)
