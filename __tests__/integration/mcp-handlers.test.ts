...
async function runTests() {
  ...
  await testFunction("resources/list and read work", async () => {
    ...
    assert.ok(listed.resources.find((resource) => resource.uri === "thailaw://server-config"));
    const config = await client.readResource({ uri: "thailaw://server-config" });
    const parsed = JSON.parse(config.contents[0].text as string);
    assert.equal(parsed.serverInfo.name, "phattja/mcp-thailaw");
    const help = await client.readResource({ uri: "thailaw://usage-guide" });
